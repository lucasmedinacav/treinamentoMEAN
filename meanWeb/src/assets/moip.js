...
(function() {
	...
	var MoipWidget = function(json, token) {
    ...
    setarToken = function(data, token) {
		data["url"] = URL + "/Instrucao.do?token=" + token;
	};
   ...
   concatZeros = function(cvv, maxPositions) {
	...
	var parcelamento;
	var MoipUtil = {
			calcularParcela: function(settings, token) {
				parcelamento = settings;
				var request = {token, "instituicao" : parcelamento.instituicao };
				var conf = {
